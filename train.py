...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch_restart', default=0, type=int, help='Epoch to restart training from')
    parser.add_argument('--estimator', default='jsd', type=str, help='MI estimator type: jsd, dv or infonce')
    parser.add_argument('--prior', default='uniform', type=str, help='Prior Distribution for matching: uniform, normal or dirichlet')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    batch_size = args.batch_size
    num_epochs = args.num_epochs
    epoch_restart = args.epoch_restart
    estimator = args.estimator
    prior = args.prior

    root = Path(r'models/' + estimator + "_" + prior)
    print(root)
    # also save the args to txt file in path to remember the configuration of training
    ...
    loss_fn = Objective(0, 1, 0.1).to(device)
    encoder_optim = Adam(encoder.parameters(), lr=1e-4)
    loss_optim = Adam(loss_fn.parameters(), lr=1e-4)

    # Continue training from epoch "epoch_restart"
    ...
    for epoch in range(epoch_restart + 1, num_epochs):
        ...
        # every 10 epochs save encoder and loss weights
        if epoch % 10 == 0:
        ...
        if epoch > 1:
            with open(root / Path('loss.pickle'), 'rb') as handle:
                loss_dict = pickle.load(handle)
                loss_dict[str(epoch)] = stats.mean(train_loss[-20:])
            with open(root / Path('loss.pickle'), 'wb') as handle:
